# Chapter 5 — Talking to MetaTrader 5
...
_198 loc · 1 classes · 0 functions · 2 imports_
...
1. Assigns <code>self.login</code> = <code>login</code>.
2. Assigns <code>self.password</code> = <code>password</code>.
3. Assigns <code>self.server</code> = <code>server</code>.
4. Assigns <code>self.terminal_path</code> = <code>terminal_path</code>.
5. Assigns <code>self.connected</code> = <code>False</code>.
...
1. Assigns <code>init_params</code> = <code>{}</code>.
2. <b>if</b> <code>self.terminal_path and 'exe' in self.terminal_path</code>: branches conditionally.
3. <b>if</b> <code>not mt5.initialize(**init_params)</code>: branches conditionally.
4. <b>if</b> <code>not self.login or not self.password or (not self.server)</code>: branches conditionally.
5. Assigns <code>authorized</code> = <code>mt5.login(self.login, password=self.password, server=self...</code>.
6. <b>if</b> <code>authorized</code>: branches conditionally (with an <b>else</b>/elif arm).
7. <b>return</b> <code>self.connected</code>.
...
1. Calls <code>mt5.shutdown(...)</code> for its side effect.
2. Assigns <code>self.connected</code> = <code>False</code>.
...
2. <b>if</b> <code>self.terminal_path and 'exe' in self.terminal_path</code>: branches conditionally.
3. <b>if</b> <code>not mt5.initialize(**init_params)</code>: branches conditionally.
4. <b>if</b> <code>self.login and self.password and self.server</code>: branches conditionally.
5. Imports <code>time</code> (lazy import inside the function).
6. Assigns <code>to_timestamp</code> = <code>time.time() + 86400</code>.
7. <b>if</b> <code>from_date</code>: branches conditionally (with an <b>else</b>/elif arm).
8. <b>try</b> block with 1 <b>except</b> clause.
9. <b>if</b> <code>deals is None</code>: branches conditionally.
10. <b>return</b> <code>deals</code>.
...
##### `MT5Connector.get_deals_by_position`
...
1. <b>if</b> <code>not self.connected</code>: branches conditionally.
...
1. <b>if</b> <code>not self.connected</code>: branches conditionally.
2. <b>try</b> block with 2 <b>except</b> clauses.
...
1. <b>if</b> <code>not self.connected</code>: branches conditionally.
2. <b>if</b> <code>not mt5.symbol_select(symbol, True)</code>: branches conditionally.
3. Assigns <code>symbol_info</code> = <code>mt5.symbol_info(symbol)</code>.
4. <b>if</b> <code>symbol_info is None</code>: branches conditionally.
5. <b>if</b> <code>order_type.upper() == 'BUY'</code>: branches conditionally (with an <b>else</b>/elif arm).
6. Assigns <code>request</code> = <code>{'action': mt5.TRADE_ACTION_DEAL, 'symbol': symbol, 'volu...</code>.
7. Assigns <code>point</code> = <code>symbol_info.point</code>.
8. <b>if</b> <code>sl_points</code>: branches conditionally.
9. <b>if</b> <code>tp_points</code>: branches conditionally.
10. Assigns <code>result</code> = <code>mt5.order_send(request)</code>.
11. <b>if</b> <code>result.retcode != mt5.TRADE_RETCODE_DONE</code>: branches conditionally.
12. Calls <code>print(...)</code> for its side effect.
13. <b>return</b> <code>True</code>.
...
_2285 loc · 1 classes · 2 functions · 12 imports_
...
##### `MT5Automator._get_cached_terminal_path`
...
##### `MT5Automator._cache_successful_path`
...
##### `MT5Automator.monitor_connection`
...
##### `MT5Automator.ensure_session_integrity`
...
##### `MT5Automator.attempt_reconnection`
...
##### `MT5Automator.check_connection_health`
...
##### `MT5Automator.verify_connection`
...
##### `MT5Automator._verify_symbol_tick_data`
...
##### `MT5Automator.is_autotrading_enabled`
...
##### `MT5Automator._get_symbol_variations`
...
##### `MT5Automator._log_available_symbols`
...
##### `MT5Automator.get_connected_symbol`
...
##### `MT5Automator.get_supported_filling_modes`
...
##### `MT5Automator._calculate_sl_tp_price`
...
6. Assigns <code>point_value</code> = <code>1.0</code>.
7. Calls <code>logging.info(...)</code> for its side effect.
8. <b>if</b> <code>sl_points and float(sl_points) &gt; 0</code>: branches conditionally.
9. <b>if</b> <code>tp_points and float(tp_points) &gt; 0</code>: branches conditionally.
10. <b>return</b> <code>(sl_price, tp_price)</code>.
...
##### `MT5Automator.check_connection_and_disconnect_if_needed`
...
##### `MT5Automator._close_mt5_processes`
...
##### `MT5Automator.get_trades_today_count`
...
##### `MT5Automator.get_orphaned_mt5_positions_by_account`
...
##### `MT5Automator.close_orphaned_positions_by_account`
...
##### `MT5Automator.get_orphaned_mt5_positions`
...
##### `MT5Automator.close_orphaned_positions`
...
##### `MT5Automator.force_close_trade`
...
##### `MT5Automator.get_daily_trade_count`
...
##### `MT5Automator.get_daily_trade_count_by_account`
...
##### `MT5Automator.reset_daily_trade_count`
...
##### `MT5Automator.reset_daily_trade_count_by_account`
...
##### `MT5Automator.get_historical_profits_by_account`
...
##### `MT5Automator.get_historical_profits`
...
##### `MT5Automator.close_orphaned_trades`
...
##### `MT5Automator.extract_tradovate_account_from_comment`
...
##### `MT5Automator.get_trades_by_tradovate_account`
...
##### `MT5Automator.debug_symbol_info`
...
##### `MT5Automator.should_close_trades_for_rollover`
...
##### `MT5Automator.close_trades_for_rollover`
